...
@export var VARIABLE_JUMP_MULTIPLIER := 1.0
@export var FALL_MULTIPLIER := 1.25
@export var LOW_JUMP_MULTIPLIER := 3.0
@export var COYOTE_TIME := 0.1
@export var JUMP_BUFFER_TIME := 0.15

@export_group("Wall")
@export var WALL_JUMP_VELOCITY := Vector2(180, -280)
@export var WALL_SLIDE_SPEED := 50.0
@export var WALL_JUMPING_DURATION := 0.175
...
func apply_variable_jump(delta):
	if velocity.y < 0:
		velocity.y += gravity * ((VARIABLE_JUMP_MULTIPLIER if Input.is_action_pressed("jump") else LOW_JUMP_MULTIPLIER) - 1.0) * delta
	elif velocity.y > 0:
